...
3. 延伸第二個結果，若沒有循環的存在，他們倆就可能沒辦法會合，但若是給予循環的存在，等同強制添加$mod\ \lambda$運算來對$H$和$T$這兩者進行同餘的計算以此
判斷他們是否會合（如下式），而如果他們都擁有相同的餘數，便是代表在循環中會合；但如果都不是一樣，便是還未會合。
...
那麼這個新位置會不會影響與烏龜會合的情況呢？只要我們按照圖上位置來模擬他們移動，最後會發現他們的確會在同一點會合，只是位置變成第$\lambda-1$個
位置，在這裡會是循環中編號9的位置。 
